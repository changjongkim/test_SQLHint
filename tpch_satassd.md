...
본 보고서는 MySQL 8.0 환경(SSD 기반)에서 TPC-H (Scale Factor 30) 벤치마크 쿼리의 **Baseline(Cold Start)** 성능과 **Optimizer Hints (Hint 01, 02, 04, 05)** 를 적용했을 때의 최적 성능을 비교 분석한 결과입니다.

분석 결과, **전체 22개 쿼리 중 약 45% (10개 쿼리)에서 유의미한 성능 향상**이 확인되었습니다. 특히 복잡한 조인이 포함된 쿼리에서 힌트가 비효율적인 실행 계획(Full Table Scan 등)을 방지하고 인덱스 사용을 강제함으로써 **최대 70% 이상의 수행 시간 단축**을 달성했습니다. 반면, 일부 쿼리(Q9)에서는 힌트가 오히려 쿼리 최적화기(Optimizer)의 효율적인 판단을 방해하여 성능이 저하되는 현상도 관찰되었습니다.
...
## 3. 쿼리별 상세 분석 및 리즈닝 (Reasoning)

각 쿼리의 성능 변화 원인을 데이터베이스 실행 계획 및 쿼리 특성을 바탕으로 분석했습니다.
